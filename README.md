...
Turn a right Joy-Con into a lightweight voice-and-edit controller for Codex Desktop.

> 当前版本 / Current version: `v0.1.0`
> 项目面向 macOS + Codex Desktop + `Joy-Con (R)` 工作流，不隶属于 Nintendo、OpenAI 或 Apple。
> This project is built for the macOS + Codex Desktop + `Joy-Con (R)` workflow and is not affiliated with Nintendo, OpenAI, or Apple.

![Right Joy-Con keymap / 右手 Joy-Con 键位图](docs/assets/keymap-readme-v2.png)

## 概述 / Overview

`JoyCon Codex Helper` 是一个 macOS 菜单栏小工具，让你用右手 Joy-Con 操作 Codex Desktop 的常用输入动作。
`JoyCon Codex Helper` is a macOS menu bar utility that lets you drive common Codex Desktop input actions with a right Joy-Con.
...
![Voice workflow / 语音到 Codex 的工作流](docs/assets/workflow-readme-v2.png)

## 默认键位 / Default Mapping

| Joy-Con 按键 | 动作（中文） | Action (English) |
| --- | --- | --- |
| `ZR` 长按 | `Control + M`，用于 Codex 语音听写 | Hold `Control + M` for Codex dictation |
| `X` / `B` | 上 / 下 | Up / Down |
| `Y` / `A` | 左 / 右 | Left / Right |
| `R` | `Return` | `Return` |
| `+` | `Backspace`，支持长按连续删除 | `Backspace` with repeat while held |
| `R3` | 尝试切换 Plan mode | Attempt Plan mode toggle |
| `Home` | 触发 controller recovery scan | Trigger controller recovery scan |
| 右摇杆方向 | 默认禁用 | Disabled by default |
| `SL` / `SR` | 默认禁用 | Disabled by default |

## 安装与运行 / Install and Run

前置条件 / Requirements:

- macOS 14 或更新版本 / macOS 14 or newer
- 已安装 Xcode Command Line Tools，且 `swift` 可用 / Xcode Command Line Tools with `swift`
- 右手 `Joy-Con (R)` 已通过蓝牙连接到 Mac / A right `Joy-Con (R)` paired to your Mac over Bluetooth
- 已安装 Codex Desktop / Codex Desktop installed
...
安装短命令 `jch` / Optional short command `jch`:
...
1. 在 macOS 蓝牙设置中连接右手 `Joy-Con (R)`。
   Pair the right `Joy-Con (R)` in macOS Bluetooth settings.
2. 启动 helper：`jch` 或 `swift run JoyConCodexHelper`。
   Launch the helper with `jch` or `swift run JoyConCodexHelper`.
3. 打开 Codex Desktop，并把焦点放进 composer 或 Plan 选项区域。
   Open Codex Desktop and put focus in the composer or a Plan option area.
4. 如果按键没有进入 Codex，先点菜单中的 `Open Accessibility Settings`，给 Terminal 或 helper 进程开启辅助功能权限。
   If input does not reach Codex, use `Open Accessibility Settings` and grant Accessibility access to Terminal or the helper process.
5. 如果 Joy-Con 连接异常，点 `Open Bluetooth Settings` 检查蓝牙连接。
   If the Joy-Con connection looks wrong, use `Open Bluetooth Settings`.
6. 按住 `ZR` 说话，松开停止听写。
   Hold `ZR` to speak, release to stop dictation.

![Menu bar entry / 菜单栏入口示意图](docs/assets/menu-readme-v2.png)
...
主要配置文件 / Main configuration files:

- `config/profiles/layout-v1-default.json`
  物理按键到 action 的映射 / Physical button to action mapping
- `config/bindings/default-actions.json`
  action 到键盘事件的映射 / Action to keyboard event mapping
- `config/runtime/input-aliases.json`
  `GameController` 与 raw HID 的运行时 alias / Runtime aliases for `GameController` and raw HID
- `config/runtime/controller-preferences.json`
  控制器选择与运行时偏好 / Controller selection and runtime preferences
- `config/runtime/target-apps.json`
  目标应用 bundle id / Target app bundle IDs
...
素材来源与说明见 / Attribution details: [docs/assets/ATTRIBUTION.md](docs/assets/ATTRIBUTION.md)
